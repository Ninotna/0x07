export class RecipeCard
{
	...
	createRecipeCard()
	{
		...
		timeBadge.classList.add('absolute', 'top-2', 'right-2', 'bg-yellow-400', 'text-white', 'text-sm', 'px-2', 'py-1', 'rounded-full', 'font-bold');
		...
		title.classList.add('text-2xl', 'font-bold', 'mb-4'); // Taille et espacement du titre
		content.appendChild(title);

		// Sous-titre "Recette"
		const recetteSubtitle = document.createElement('h4');
		recetteSubtitle.textContent = 'RECETTE';
		recetteSubtitle.classList.add('uppercase', 'text-sm', 'font-semibold', 'text-gray-600', 'mb-2');
		content.appendChild(recetteSubtitle);

		// Description de la recette
		const description = document.createElement('p');
		description.textContent = this.recipe.description;
		description.classList.add('text-gray-700', 'mb-4'); // Couleur grise pour le texte
		...
		ingredientsSubtitle.classList.add('uppercase', 'text-sm', 'font-semibold', 'text-gray-600', 'mb-2');
		content.appendChild(ingredientsSubtitle);

		// Liste des ingrédients
		const ingredientsGrid = document.createElement('div');
		ingredientsGrid.classList.add('grid', 'grid-cols-2', 'gap-y-2'); // Grid layout for ingredients
